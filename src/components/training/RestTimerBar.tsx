...
import { bell } from "@/lib/training/sound";
...
export function RestTimerBar({ active, remaining, total, onAdd, onSkip }: RestTimerBarProps) {
  ...
  return (
    <div role="status" aria-live="polite" className="fixed inset-x-0 bottom-0 z-40 border-t border-primary/30 bg-korev-deep/95 backdrop-blur-md">
      ...
      <div className="container flex max-w-3xl items-center gap-4 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="min-w-0 flex-1">
          ...
          <p className="korev-metric text-3xl leading-none">{formatClock(remaining)}</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => onAdd(15)}>+15 s</Button>
