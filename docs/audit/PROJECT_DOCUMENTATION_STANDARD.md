...
| Analyse vidéo de sparring | Implémentée | `src/components/sparring/SparringAnalysisV2.tsx` (+ `SparringUploadPanel`, `SparringProcessing`, `SparringResults`), `src/utils/motionSheetExtractor.ts`, `supabase/functions/analyze-sparring/index.ts`, table `sparring_analyses`, bucket `sparring-videos` | Planches de mouvement 2×2 horodatées extraites côté client (jusqu’à 48, 4 instants à 0,25 s), identification facultative de l’athlète, vision LLM côté Edge via **tool/function calling** (`submit_sparring_analysis`), profils par discipline (boxe, kickboxing, muay thai, MMA, BJJ, karaté/taekwondo) avec règles d’invalidation des métriques non pertinentes, modèle `google/gemini-2.5-pro` par défaut et `flash` en `qualityMode='fast'`, gating serveur, validation/normalisation stricte du JSON |
...
| Partage d’analyse sparring | Export PDF uniquement | `src/components/sparring/SparringPDFExport.tsx` | Partage par lien retiré (aucune page publique ni jeton de partage) |
...
| Paywall fonctionnel | Implémentée | `src/components/FeaturePaywall.tsx`, `src/components/PaywallDialog.tsx`, `src/hooks/useFeatureGate.tsx` | Hook utilitaire combinant `useFeatureAccess.runFeatureWithTracking` + état d’ouverture du paywall (`gate()` → boolean + setter d’overlay) |
